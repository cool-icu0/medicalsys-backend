...
import com.example.medicalsysbackend.domain.request.MessageRequest;
...
import static com.example.medicalsysbackend.constant.ChatConstant.*;
...
public class WebSocket {
    ...
    public void sendMessage(String message) throws IOException {
        ...
        System.out.println("这个是message"+message);
    }
    ...
    private void hallChat(User user, String text, Integer chatType) {
        ...
        fromWebSocketVo.setId(user.getUserId());
        ...
        savaChat(user.getUserId(), null, text, null, chatType);
    ...
    private void privateChat(User user, Long toId, String text, Integer chatType) {
        ...
        if (toSession != null) {
            MessageVo messageVo = chatService.chatResult(user.getUserId(), toId, text, chatType, DateUtil.date(System.currentTimeMillis()));
            User loginUser = (User) this.httpSession.getAttribute(USER_LOGIN_STATE);
            if (loginUser.getUserId() == user.getUserId()) {
                messageVo.setIsMy(true);
            }
     ...
     * @param chatType 聊天类型
     */
    private void savaChat(Integer userId, Long toId, String text, Long teamId, Integer chatType) {
        if (chatType == PRIVATE_CHAT) {
...
//            }
        ...
        Chat chat = new Chat();
        chat.setFromId(Long.valueOf(userId));
